...
import { Bot, FileCheck2, ScrollText, Settings, Zap } from 'lucide-react'
...
export default function Sidebar() {
  ...
  const { vendor, operator, proposals, backendOnline } = useStore()
  ...
  return (
    <aside className="fixed left-0 top-0 bottom-0 w-[260px] z-20 flex flex-col border-r border-line bg-[#060a13]/90 backdrop-blur-xl">
      <div className="h-16 flex items-center gap-3 px-6 border-b border-line">
        ...
        <span className="text-[10px] font-mono font-medium text-accent-cyan bg-accent-cyan/10 px-1.5 py-0.5 rounded-md ml-auto">
          demo
        </span>
      </div>
      ...
      <div className="border-t border-line p-4 space-y-3">
        <div className="glass-sm p-2.5 space-y-1">
          <div className="flex items-center justify-between">
            ...
            <span
              className={`w-1.5 h-1.5 rounded-full ${
                backendOnline ? 'bg-accent-green' : 'bg-accent-amber'
              }`}
            />
          </div>
          <div className="flex items-center gap-2">
            ...
            <div className="min-w-0 flex-1">
              <span className="text-xs font-semibold text-text truncate block">{vendor.name}</span>
              <span className="text-[9px] font-mono text-text-dim">{vendor.id}</span>
            </div>
          </div>
        </div>
        ...
        <div className="flex items-center gap-2.5">
          ...
          <div className="flex-1 min-w-0">
            ...
            <span
              className={`inline-block text-[9px] font-bold uppercase px-1.5 py-0.5 rounded-md mt-0.5 ${
                roleColors[operator.role] || roleColors.viewer
              }`}
            >
              {operator.role}
            </span>
