...
def _tool(name: str, args: dict | None = None, ctx: VendorContext | None = None):
    run_ctx = ToolRunContext()
    raw = invoke_read_tool_direct(name, args or {}, ctx or _acme_ctx(), run_ctx)
...
def test_games_repo_scoped_to_vendor():
    with SyncSessionLocal() as session:
        acme = GamesRepo(session).list_games(vendor_id="v_acme")
        beta = GamesRepo(session).list_games(vendor_id="v_beta")
...
def test_trial_users_badminton_only_acme():
    with SyncSessionLocal() as session:
...
def test_revenue_today_all_games_acme():
    with SyncSessionLocal() as session:
        rev = RevenueRepo(session).get_revenue(
...
def test_revenue_cross_vendor_isolation():
    with SyncSessionLocal() as session:
        acme = RevenueRepo(session).get_revenue(vendor_id="v_acme")
        beta = RevenueRepo(session).get_revenue(vendor_id="v_beta")
    assert acme["net_cents"] == 128_500
    assert beta["net_cents"] == 9_900


def test_search_users_does_not_leak_other_vendor():
    with SyncSessionLocal() as session:
        hits = UsersRepo(session).search(vendor_id="v_acme", query="eve")
    assert hits == []
    with SyncSessionLocal() as session:
        hits = UsersRepo(session).search(vendor_id="v_beta", query="eve")
    assert len(hits) == 1
    assert hits[0]["id"] == "u_beta_eve"


def test_get_user_other_vendor_returns_none():
    with SyncSessionLocal() as session:
        user = UsersRepo(session).get_user(vendor_id="v_acme", user_id="u_beta_eve")
    assert user is None
...
def test_tool_list_trial_users_badminton():
    data, run_ctx = _tool("list_trial_users", {"game_slug": "badminton", "limit": 20})
    assert data["count"] == 2
...
def test_tool_get_revenue():
    data, run_ctx = _tool("get_revenue", {})
    assert data["found"] is True
    assert data["net_cents"] == 128_500
    assert run_ctx.blocks and run_ctx.blocks[0]["type"] == "kpi"


def test_tool_list_games():
    data, _ = _tool("list_games", {})
    slugs = {g["slug"] for g in data["games"]}
    assert slugs == {"badminton", "cricket"}


def test_tool_search_users_alice():
    data, _ = _tool("search_users", {"query": "alice"})
    assert data["count"] == 1
    assert data["users"][0]["id"] == "u_alice"


def test_tool_get_user_with_memberships():
    data, _ = _tool("get_user", {"user_id": "u_alice"})
    assert data["found"] is True
...
def test_tool_get_membership():
    data, _ = _tool("get_membership", {"user_id": "u_alice", "game_slug": "badminton"})
    assert data["found"] is True
    assert data["membership"]["plan"] == "trial"


def test_tool_vendor_summary():
    data, run_ctx = _tool("get_vendor_summary", {})
...
def test_tool_beta_cannot_see_acme_trials():
    data, _ = _tool("list_trial_users", {"game_slug": "badminton"}, ctx=_beta_ctx())
    # Beta has no badminton game / trials
    assert data["count"] == 0


def test_tool_beta_sees_own_revenue_only():
    data, _ = _tool("get_revenue", {}, ctx=_beta_ctx())
    assert data["found"] is True
